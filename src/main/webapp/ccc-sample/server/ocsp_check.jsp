...
<%  
	...
	if(nRet==0) 
	{
		...
		if(nRet==0) 
		{
			...
			out.println("ServerCertDN : " + ServerCertDN + "<br>");
			out.println("ServerCertDN.getBytes() : " + ServerCertDN.getBytes() + "<br>");

			nRet = CCertificate.ValidateCertOCSP(CVerifier.certbuf, CVerifier.certlen, policies, 3, ServerCertPwd.getBytes(), ServerCertDN.getBytes());
			
			
			if(nRet == 0){  // ocsp 인증서 검증 성공
				out.println("ocsp 검증 : 성공 <br>");

			}
			else {

				out.println("ocsp 검증 실패 후 인증서 만 검증하기!!<br>");
