...
<%  
	...
	{
		byte[] test =null;
		int len = CBase64.contentlen;
		for(int i=0;i<len;i++){
			test = CBase64.contentbuf;
			test[i] = (byte)5;
		
			nRet = CVerifier.VerSignedData(test, test.length);
			if (nRet == 0)
			{
				out.println("["+i+"]서명 검증  성공 : "  + new String(CVerifier.contentbuf));
			}
			else{
				out.println("["+i+"]서명 검증  실패 : " + CVerifier.errmessage );	
			}
		}
